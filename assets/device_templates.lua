...
local gpio = require("gpio")  -- 取消注释，引入GPIO模块
local dht = require("dht")    -- 引入DHT模块（如果使用）
local pwm = require("pwm")    -- 引入PWM模块（如果使用）
...
local base_template = {
    ...
    update = function(self, new_params)
        ...
        self.updated_at = os.time()
        log.info(TAG, "设备更新: %s [%s]", self.id, self.type)
    end,
    ...
    set_status = function(self, status)
        if self.status ~= status then
            ...
            log.info(TAG, "设备状态变更: %s [%s] -> %s", self.id, self.type, status)
        end
    end,
...
local generic_sensor = DeviceTemplate.create_template(
    ...
    },
    {
        ...
        start_polling = function(self)
            ...
            self.poll_timer = timer.setInterval(self.poll_interval, poll_func)
    ...
    },
    {
        ...
        read_value = function(self)
            if not self.gpio then
                log.error(TAG, "DHT11传感器缺少GPIO配置: %s", self.id)
                return nil
            end
            
            local ok, temp, humi = dht.read(self.gpio, dht.DHT11)
            if ok then
                return {
                    temperature = temp,
                    humidity = humi
                ...
                log.error(TAG, "读取DHT11传感器失败: %s", self.id)
                return nil
            end
        end,
    }
...
local led_actuator = DeviceTemplate.create_template(
    ...
    },
    {
        ...
        blink = function(self, interval, count)
            ...
            self.blink_timer = timer.setInterval(interval, toggle_func)
